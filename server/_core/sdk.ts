...
class SDKServer {
  ...
  private getSessionSecret() {
    const secret = ENV.cookieSecret || "arcway-default-jwt-secret-key-32-chars-long";
  ...
  async verifySession(
    ...
    try {
      ...
      return {
        openId,
        appId: typeof appId === "string" && appId ? appId : "arcway",
        name,
      };
    } catch (error) {
      ...
      try {
        ...
        if (!ENV.oAuthServerUrl) {
          return {
            ...
            name: session.name || "ARCWAY Reviewer",
            email: "reviewer@arcway.trade",
            loginMethod: "local",
            role: "reviewer",
